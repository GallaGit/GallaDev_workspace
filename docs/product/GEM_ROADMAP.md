...
| 1.2 | ✅ Migración `20260922000000_roles_rls.sql` aplicada en Supabase (2026-09-22, verificada: `profiles` existe, anónimo sin acceso, app vía `service_role` intacta): enum `app_role` (`Admin`/`Seller`/`Viewer`), tabla `profiles`, policies por rol en `leads`/`lead_activities` | `authenticated` sin rol no lee/escribe leads ajenos; proveedor (`leadsDbProvider`) intacto |
